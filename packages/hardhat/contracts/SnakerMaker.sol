...
using Strings for uint256;
  ...
  function generateSVGofTokenById(uint256 id) public view returns (Snake memory, string memory) {
    string memory seed = Strings.toString(id * 6969);
    Snake memory snk;
    ...
    if (randVal.x[0] < 30) {
      ...
      //snk.bg_color = randomColor(string(abi.encodePacked(seed, "bgcolor")), 18, 65, 70, 100, 25, 65); 
    } else {
      // space
      snk.bg_color = randomColor(string(abi.encodePacked(seed, "bgcolor")), 0, 359, 80, 100, 25, 60); 
...
/*     uint8[5] memory thickness = [uint8(12), 21, 30, 39, 48];
    uint8[5] memory head = [uint8(24), 30, 39, 42, 48];
    uint8[5] memory head_mark = [uint8(10), 12, 14, 16, 18];

    snk.thickness = thickness[snk.thickIdx];
    snk.head = head[snk.thickIdx];
    snk.head_mark = head_mark[snk.thickIdx]; */
...
/*     if (randVal.x[0] < 1){
      snk.skin_pattern_freq = "0.02";
    } else if (randVal.x[0] < 2) {
      snk.skin_pattern_freq = "0.04";
    } else if (randVal.x[0] < 3) {
      snk.skin_pattern_freq = "0.06";
    } else if (randVal.x[0] < 4) {
      snk.skin_pattern_freq = "0.08";
    } else if (randVal.x[0] < 5) {
      snk.skin_pattern_freq = "0.1";
    } else {
      snk.skin_pattern_freq = "0.15";
    } */
